...
const WeeklyReports = ({darkMode}) => {
    ...
    useEffect(() => {
        ...
        const getAllReports = async () => {
            try {
                setLoading(true);
                const weeklyResponse = await axiosInstance.get('/api/reports/get-all-weakly/');
                setWeeklyReports(weeklyResponse.data.data);
                if (weeklyResponse.data.data && weeklyResponse.data.data.length > 0) {
